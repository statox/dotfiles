...
# --- Auto-(de)activate micromamba env based on current path ---
function chpwd_auto_micromamba() {
    if [[ $PWD == ~/projects* ]]; then
        if [[ -z $CONDA_PREFIX ]]; then
            micromamba activate experiments
        fi
    else
...
# Toogle mamba experiment when changing the path in an interactive session
add-zsh-hook chpwd chpwd_auto_micromamba
# Check for activation on new shell
chpwd_auto_micromamba
# }}}
